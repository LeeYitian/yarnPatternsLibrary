...
> 🟡 手動 tag 半邊**設計已定案、待實作**（本輪討論定稿）：資料格式、孤兒策略、快取、輸入介面、卡片/篩選整合、onboarding 併入方式皆已拍板，見 §6（`files.md`／`links.md` 啟用）、§7.1（篩選整合）、§9（決策 T20–T27）、§11.6（編輯彈窗與 chip 輸入）。實作尚未動工。
...
2. **手動 tag（設計已定案、待實作）**：使用者自訂（`#送禮`、`#未完成`），本機檔案與 URL 條目都能標。輸入介面、儲存、孤兒策略、篩選整合皆已拍板（§6、§7.1、§9 T20–T27、§11.6），只差寫程式。
...
- **`About.md`**：本輪已同步——§5（`files.md`／`kv.files` 由「延後」改「設計定案待實作」）、§6.1（唯讀語意澄清）、§10（手動 tag 待做清單）。實作完成後再把「待實作」字樣改為「已實作」。
