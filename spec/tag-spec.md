...
> ✅ 狀態：**已實作**——「資料夾路徑 → 自動 tag」（§4）、folder-tag 開關（§4a）、tag 篩選（§7）、設計規格（§11）皆已落地（`js/tags.js`＋篩選區 `#filterbar`＋onboarding「資料夾標籤」步驟）。**手動 tag 延後**（§6 只定格式、§8 延後清單）。本文件轉為行為真相＋後續維護依據。
...
- **URL 條目沒有自動 tag**：選中任一 tag 時，URL 卡片必然全數被濾出（AND 交集的自然結果，屬**預期行為**）。手動 tag 上線後才會改變。
...
> **不放工具列巡禮**：tag 介面在桌面與手機都與搜尋欄同屬一個篩選區，開關選完接著介紹（`onboarding-spec.md` §9.1 子流程 ③）。桌面指 header 下的篩選區；手機指 dock 第 4 顆「篩選」鈕。**無子資料夾（純說明形式）時不播此 spotlight**。
