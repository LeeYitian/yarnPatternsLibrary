...
**一律出現於新使用者的 onboarding 序列**（步驟數固定，不因資料夾結構增減），形式依偵測結果分兩種：

- **有子資料夾** → **互動步驟**（🖱 換你試試，成為第二個互動步驟）：說明子資料夾檔案已攤平顯示、資料夾名可當標籤篩選，並讓使用者**當場按開 ／ 關 folder-tag**（真的生效、寫入 `tag-spec.md` §4a 的持久偏好）。比照 Step 4「試試新增網址」的「做了會生效」。
- **無子資料夾（平資料夾）** → **純說明步驟**（👀 看過就好）：只告知有此功能，**不給開 ／ 關選擇**。
...
- **O9 例外**（§6）：不再是「v1 只有『試試新增網址』一步能互動」；新增「資料夾標籤」為第二個互動步驟。
