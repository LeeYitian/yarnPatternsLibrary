...
2. 重新讀一次 `links.md`,把你在別的地方
   改過的網址收藏內容讀進來

✅ 什麼時候要按:
- 在檔案總管動了檔案(新增／改名／刪掉)
- 在 VS Code 改了 `links.md`,
  想立刻在網頁上看到結果
- 打開網頁後,懷疑畫面跟資料夾裡實際的東西不一樣

🛟 不會弄丟你的收藏:
在網頁外面改 `links.md` 不會造成資料遺失。
就算忘了按重新整理,直接在網頁裡新增網址,
你這次沒動到的那幾筆收藏,
外面改過的內容都會自動保留。
```
