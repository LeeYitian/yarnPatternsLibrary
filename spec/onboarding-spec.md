...
> ✅ **狀態：已實作(v1)**。目前共 **7 大步**教學(混合對話框 + spotlight)已落地(獨立 controller,`OB_SEEN_KEY = "wlib-onboarding-seen"`,`maybeAutoOnboarding()` 首次觸發、設定選單 `#replayOnboarding` 重看入口)。§9 的「資料夾標籤」步驟與單步播放(`playFoldertagStep()`)**已實作**;原 v1 的「萬一 links.md 壞掉」步**已移除**(`.broken` 敘述誤導,見 `About.md` §10、§9.6)。步序真相以 §9.6 表為準。本文件轉為「行為真相 + 後續維護依據」;教學內容隨 app 功能擴充仍需同步維護(見 §7 ⚠️)。
>
...
>
> ⚠️ 本節是 v1 的原始拆解、**保留原編號未重排**；實際播放順序以 §9.6 的表為準（目前共 **7 大步**）。與 v1 相比有兩處變動：（1）原 **Step 2「萬一 links.md 壞掉」已移除**——它描述的 `.broken` 自動備份實際上不會觸發（`parseLinks`／`parseFiles` 容錯、永不 throw，詳見 `About.md` §10），留著會誤導；（2）已插入**新的「資料夾標籤」步驟**（規格見 §9.1、文案真相見 `tag-spec.md` §9a）。Step 1a 文案也已改為支援子資料夾。

### 開場：歡迎（封面）

模式：👀 看過就好（封面，**不顯示徽章、不計入下方 7 步的步驟點**）
...
### ~~Step 2：萬一 links.md 壞掉~~（已移除）

原步驟宣稱「`links.md` 打錯格式 → 網頁自動從副本救回、壞檔備份成 `links.md.broken-{當下時間}`、可打開 `.broken` 撿回」。**實際不成立**：`parseLinks`／`parseFiles` 是容錯解析器、永不 throw，`.broken` 那條路不會被觸發（壞一行 → 該行靜默丟棄、下次 CRUD 覆寫；整份壞 → parse 成空並覆蓋 `kv` 快取）。此步敘述誤導，故移除。行為真相與後續處理見 `About.md` §10。
...
- `url-spec.md` 核心設計變動（refresh 職責、cache 同步方向、災難復原行為）→ review §4 Step 3（refresh）／ Step 4（新增網址流程）。⚠️ 災難復原若真的動到（例如讓 parser 對壞檔 throw、`.broken` 備份改為會觸發），要回頭考慮是否恢復「links.md 壞掉」的教學步（現已移除，見 §9.6、`About.md` §10）
...
**一律出現於新使用者的 onboarding 序列**（步驟數固定，不因資料夾結構增減）。**位置：插在「重新整理」之後、「試試新增網址」之前**（v1 原編號下成為新 Step 4；`OB_TOTAL` 當時 7 → 8）。⚠️ 之後又移除誤導的「links.md 壞掉」步，`OB_TOTAL` 回到 **7**，實際步序見 §9.6。形式依偵測結果分兩種：
...
> 隨子資料夾 ／ tag 上線一起調整。v1 原為 7 大步；加入「資料夾標籤」一度 **7 → 8**，之後**移除誤導的「links.md 壞掉」步**（`.broken` 自動備份實際不觸發，見 `About.md` §10）**又回到 7**（`OB_TOTAL`＝7，步驟指示器一併更新）。粗體為對 v1 的變更。

| 新步序 | 內容 | 對 v1 的變更 |
| --- | --- | --- |
| 1 | 兩種收藏 | Step 1a「檔案」段改支援子資料夾（文案 `subfolder-spec.md` §9a） |
| 2 | 重新整理 | **保留原對話框說明，額外加一個 spotlight 指向設定選單的「重新整理」鈕**（變更 1） |
| 3 | **資料夾標籤（新步驟）** | **新增**：互動時開 ／ 關 folder-tag（文案 `tag-spec.md` §9a）；**手動 tag 純說明（可手動加 tag、tag 存放位置；孤兒警語改放編輯彈窗小灰字）＋篩選區 spotlight 併入本步**（§9.1 子流程 ③④） |
| 4 | 試試新增網址（互動） | 位置後移 |
| 5 | 工具列巡禮 | **刪掉「設定選單裡有『重新整理』和『更換資料夾』」那顆 spotlight**（變更 2；兩者已各自有 spotlight＝步序 2／6），剩幻燈片＋dock 三顆 |
| 6 | 更換資料夾 | **保留原對話框說明，額外加一個 spotlight 指向設定選單的「更換資料夾」鈕**（變更 3） |
| 7 | 結尾 | — |
| ~~—~~ | ~~萬一 `links.md` 壞掉~~ | **已移除**（原 v1 Step 2；`.broken` 敘述誤導，`About.md` §10） |

**實作注意（spotlight 指下拉選單項）**：「重新整理」與「更換資料夾」都在頂欄設定齒輪的**下拉選單內**、平時收合。步序 2／6 的 spotlight 先**展開設定選單並鎖住**再定位（復用 §9.1-② 的 `menu: true` 機制；純說明、遮罩攔截點擊）。

**互動步驟現有兩個**（資料夾標籤 + 新增網址）→ O9 例外已記於 §9.3。篩選區 spotlight **併入步序 3「資料夾標籤」**（不放工具列巡禮，維持 §9.1 與 `tag-spec.md` §9a 的定案）。
